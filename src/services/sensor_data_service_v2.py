...
class SensorDataServiceV2:
    ...
    def _upload_sensor_data(self, device_config: Dict, processed_data: Dict[str, Optional[float]]):
        ...
        for key, value in processed_data.items():
            if value is None:
                continue
            # 跳过不在允许集合中的键（例如 turbidity_temperature）
...
class SensorDataServiceV2:
    ...
    def _upload_sensor_data(self, device_config: Dict, processed_data: Dict[str, Optional[float]]):
        ...
        for key, value in processed_data.items():
            ...
            if batch_id:
                description += f" - 批次{batch_id}"
            print(f"传感器数据上传成功: sensor_id={sensor_id}, metric={upload_metric}, value={value}, unit={upload_unit}, timestamp={timestamp_ms}")
            try:
                api_client.send_sensor_data(
                    sensor_id=sensor_id,
                    value=value,
